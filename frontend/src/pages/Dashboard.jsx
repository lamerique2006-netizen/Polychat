...
export default function Dashboard() {
  ...
  // Open sidebar on large desktop by default
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden">
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside 
            className="fixed xl:relative z-50 w-72 h-full flex-shrink-0 bg-gray-900/95 xl:bg-gray-900/80 backdrop-blur-2xl border-r border-white/5 flex flex-col shadow-xl"
            initial={{ x: -300 }}
            animate={{ x: 0 }}
            exit={{ x: -300 }}
          ...
          >
            ...
            <div className="p-3">
              <motion.button
                ...
                className="w-full bg-gradient-to-r from-brand-600 to-purple-600 hover:from-brand-700 hover:to-purple-700 text-white text-xs font-medium py-2 rounded-lg transition-all shadow-lg"
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-2 min-h-0">
              <p className="text-xs text-gray-500 mb-2 px-1 uppercase tracking-wider">Recent</p>
              <div className="space-y-1">
                {conversations.map(conv => (
                  <motion.button
                    key={conv.id}
                    onClick={() => loadConversation(conv.id)}
                    className={`w-full text-left px-2 py-1.5 rounded-lg text-xs transition group relative ${
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 text-xs flex-shrink-0"
                      >
                        🗑️
                      </button>
                    </div>
            ...
            <div className="p-3 border-t border-white/5">
              ...
              >
                ...
                <div className={`w-8 h-5 rounded-full transition ${compareMode ? 'bg-white/20' : 'bg-gray-700'} relative`}>
                  <motion.div
                    className="w-3 h-3 bg-white rounded-full absolute top-1"
                    animate={{ x: compareMode ? 16 : 4 }}
            ...
            <div className="p-3 border-t border-white/5">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 text-left min-w-0">
                        <div className="font-semibold text-xs truncate">{m.label}</div>
                        <div className="text-xs opacity-60 truncate">{m.desc}</div>
                      </div>
                      {isSelected && (
                        <motion.div 
                          layoutId={compareMode ? undefined : "activeSidebar"}
                          className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${compareMode ? 'bg-green-400' : 'bg-brand-500'}`}
                        />
                      )}
                    </motion.button>
                  )
              ...
              <div className="mt-3 pt-3 border-t border-white/5">
                <Link
                  to="/analytics"
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition"
                >
            ...
            <div className="p-3 border-t border-white/5 space-y-2">
              <div className="bg-gray-800/50 rounded-lg p-2 border border-white/5">
                <p className="text-xs text-gray-500 mb-0.5">Plan: <span className="text-white font-semibold">Free</span></p>
                <div className="text-xs text-gray-400">
                  Quota: <span className="font-bold text-brand-400">50/50</span>
                </div>
              </div>
              
              <div className="flex items-center gap-2 px-1">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-brand-600 to-purple-600 flex items-center justify-center text-white text-xs font-bold">
              ...
              <button 
                ...
                className="w-full text-xs text-gray-400 hover:text-red-400 transition py-1.5"
              >
                Sign out
              </button>
            </div>
